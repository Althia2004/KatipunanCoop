...
export default function Loans({ user, member, loans }: Props) {
    const [expandedLoan, setExpandedLoan] = useState<number | null>(null);
    const [createOpen, setCreateOpen] = useState(false);

    // Member Loan Request Form
    const createForm = useForm({
        member_id: member.id, // Hidden or auto-filled for members
        amount: '',
        purpose: '',
        term_months: '12',
        interest_rate: '3'
    ...
    const submitCreate = (e: React.FormEvent) => {
        e.preventDefault();
        // Matching the path structure in Management.tsx
            ...
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm text-zinc-500 font-medium">View your active loans and payment schedules.</p>
                    </div>
                    <button
                ...
                {loans.length === 0 ? (
                    <div className="bg-white border border-dashed border-zinc-200 rounded-2xl py-16 text-center">
                        ...
                        <h3 className="text-zinc-900 font-semibold">No active loans</h3>
                        <p className="text-zinc-500 text-sm mt-1 max-w-xs mx-auto">
                            You don't have any loan records yet. Click "Apply for a Loan" to start a new application.
                        </p>
                    </div>
                ) : (
                                    ...
                                    >
                                        <div className="flex items-center gap-4 flex-1">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs text-zinc-500">Loan ID: #{loan.id} • Issued on {new Date(loan.created_at).toLocaleDateString()}</p>
                                            </div>
                                        </div>
                                                    ...
                                                    <tbody className="divide-y divide-zinc-100 text-zinc-600">
                                                        {loan.amortizations.map((a) => (
                                                            <tr key={a.id} className="hover:bg-zinc-50/80">
                                                                <td className="px-4 py-3 font-medium">{new Date(a.due_date).toLocaleDateString('en-PH', { month: 'short', day: 'numeric', year: 'numeric' })}</td>
